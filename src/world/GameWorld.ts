...
export class GameWorld {
  ...
  syncEnemy(id: number, type: string, x: number, y: number, w: number, h: number, facing: number, dead: boolean, t: number) {
  ...
  syncPlayer(
    ...
    other.visible = false;
    if (attacking && attackKind === 'primary') {
      vfx.visible = true;
      const ox = facing * (form === 'anima' ? 36 : 40);
      vfx.position.set(c.x + ox, c.y, 8);
      vfx.scale.x = facing >= 0 ? 1 : -1;
      vfx.rotation.z = form === 'anima' ? t * 8 : 0;
    } else {
      vfx.visible = false;
    }
  }
  ...
  update(dt: number, playerCx: number, playerCy: number) {
    this.clock += dt;
    ...
    this.cam.follow(playerCx, -playerCy, this.level.width, dt);
    this.updateLabels();
  }
